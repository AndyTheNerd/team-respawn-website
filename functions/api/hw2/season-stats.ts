...
async function loadCachedSeasonStats(db: D1Database, playerId: string, seasonId: string) {
  const row = await db.prepare(
    'SELECT payload_json, fetched_at FROM player_season_stats_cache WHERE player_id = ? AND season_id = ?'
  ).bind(playerId, seasonId).first<CachedRow>();
...
async function storeSeasonStats(db: D1Database, playerId: string, gamertag: string, seasonId: string, payload: unknown) {
  const now = new Date().toISOString();
  const statements: unknown[] = [];

  statements.push(
    db.prepare(
      `INSERT INTO player_season_stats_cache (player_id, season_id, gamertag, payload_json, fetched_at)
       VALUES (?, ?, ?, ?, ?)
       ON CONFLICT(player_id, season_id) DO UPDATE SET
         gamertag = excluded.gamertag,
         payload_json = excluded.payload_json,
         fetched_at = excluded.fetched_at`
    ).bind(playerId, seasonId, gamertag, JSON.stringify(payload), now)
  );
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const urlStats = `${HALO_ENDPOINTS.SUMMARY_API_URL}/players/${encoded}/stats/seasons/${seasonId}`;
  const result = await fetchWithKeyFallback<any>(urlStats, apiKeys);

  if (result.ok) {
    const fetchedAt = await storeSeasonStats(env.DB, normalizePlayerId(gamertag), gamertag, seasonId, result.data);
