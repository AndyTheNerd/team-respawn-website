...
async function loadCached(db: D1Database, playerId: string) {
  const row = await db.prepare(
    'SELECT payload_json, fetched_at FROM player_campaign_cache WHERE player_id = ?'
...
async function storeCampaign(db: D1Database, playerId: string, gamertag: string, payload: unknown) {
  const now = new Date().toISOString();
  await db.batch([
    db.prepare(
      `INSERT INTO player_campaign_cache (player_id, gamertag, payload_json, fetched_at)
       VALUES (?, ?, ?, ?)
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const apiUrl = `${HALO_ENDPOINTS.SUMMARY_API_URL}/players/${encoded}/campaign-progress`;
  const result = await fetchWithKeyFallback<any>(apiUrl, apiKeys);

  if (result.ok) {
    const fetchedAt = await storeCampaign(env.DB, normalizePlayerId(gamertag), gamertag, result.data);
    return jsonResponse({ ...result.data, _meta: { cached: false, fetchedAt } });
  }

  if (shouldUseCache(result.error.type)) {
    const cached = await loadCached(env.DB, normalizePlayerId(gamertag));
    if (cached) {
      return jsonResponse({
        ...cached.payload,
